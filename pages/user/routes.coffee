...
module.exports = (app, db) ->
  ...
  app.post '/user/lookup_activities/:user_id', (req, res) ->
    mongo.events_by_id db, req.params.user_id, (err, resp) ->
      res.type('json')
      return res.status(400).send(JSON.stringify("Failed!")) if err
      res.status(200).send(JSON.stringify(if resp?.activities? then resp.activities else []))
  ...
  app.post '/user/lookup_todo/:user_id', (req, res) ->
    mongo.todos_by_id db, req.params.user_id, (err, resp) ->
      res.type('json')
      return res.status(400).send(JSON.stringify("Failed!")) if err
      res.status(200).send(JSON.stringify(if resp? then resp else {}))
